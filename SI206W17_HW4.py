...
ny_times_headlines_all = []

the_soup = BeautifulSoup(text_data_from_site, 'html.parser')

for headline in the_soup.find_all(class_='story-heading'):
	if headline.a:
		ny_times_headlines_all.append(headline.a.text.replace("\n", " ").strip())

	else:
		ny_times_headlines_all.append(headline.contents[0].strip())
...
for a_person in people:
	...
	the_persons_name = a_person.find("div", {"property":"dc:title"})
	the_persons_title = a_person.find("div", {"class": "field field-name-field-person-titles field-type-text field-label-hidden"})

	umsi_titles[the_persons_name.text] = the_persons_title.text
